...
function handleDutyReq(req, data, done) {
  ...
  const urlSchedulingParams = req.url.split("/")[3];
  ...
  } else if (req.method === "PATCH" && parsedUrlLength === 3) {
    return duties.updateDuty(urlParams, data, done)
  } else if (req.method === "PUT" && urlSchedulingParams === "schedule" && parsedUrlLength === 4) {
    return duties.scheduleDuty(urlParams, done)
  }
  return done(new Error("Bad request, not a POST, GET, DELETE, PATCH or PUT request"))
}
